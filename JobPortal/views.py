...
def dashboard(request):
    ...
    if profile.user_type == "recruiter":
        ...
        matched_jobs = Job.objects.filter(query).exclude(
            ...
            :10
        ]  # Limit to 10, exclude already applied jobs
        return render(
            request,
            "portal/jobseeker_dashboard.html",
            {"applications": applications, "matched_jobs": matched_jobs},
        )
...
def post_job(request):
    ...
    return render(
        request,
        "portal/post_job.html",
        {"form": form, "title": "Post", "submit": "Post"},
    )
...
def job_list(request):
    ...
    if (
        ...
        and request.user.userprofile.user_type == "jobseeker"
    ):
        applied_jobs = Application.objects.filter(jobseeker=request.user).values_list(
            "job_id", flat=True
        )
...
def manage_applications(request, job_id):
    ...
    return render(
        request,
        "portal/manage_applications.html",
...
def edit_job(request, job_id):
    ...
    return render(
        request,
        "portal/post_job.html",
        {"form": form, "job": job, "title": "Edit", "submit": "Update"},
    )
